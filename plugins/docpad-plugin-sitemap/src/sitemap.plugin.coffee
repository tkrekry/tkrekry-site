...
module.exports = (BasePlugin) ->
	...
	class SitemapPlugin extends BasePlugin
		...
		writeAfter: (opts,next) ->
			...
			# log
			docpad.log('info', 'Creating sitemap in ' + sitemapPath)
			...
			docpad.getCollection(sitemapData.collectionName).sortCollection(date:1).forEach (document) ->
				if (document.get('sitemap') isnt false) and (document.get('url').match(/vendor/g) == null) and (document.get('write') isnt false) and (document.get('ignored') isnt true)

					pageUrl = if document.get('url').match(/\.html$/g) then document.get('url') else document.get('url') + '.html'
			...
			safefs.writeFile sitemapPath, sitemap.toString(), (err) ->
				...
				# log
				docpad.log('info', "Wrote the #{sitemapData.filePath} file to: #{sitemapPath}")
